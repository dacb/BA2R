BEGIN {
	...
	if (create_table == 1)
		printf("DROP TABLE IF EXISTS %s;\nCREATE TABLE %s ( \n type VARCHAR(4) NOT NULL, \n locus VARCHAR(64) NOT NULL, \n locus_tag VARCHAR(64) NOT NULL PRIMARY KEY, \n strand CHAR NOT NULL, \n start_coord INTEGER NOT NULL, \n end_coord INTEGER NOT NULL, \n gene_symbol VARCHAR(24), \n product VARCHAR(128) NOT NULL, \n note VARCHAR(1024), \n translation TEXT);\nCREATE INDEX IF NOT EXISTS %s_locus ON %s (locus);\n CREATE INDEX IF NOT EXISTS %s_start_coord ON %s (start_coord);\n CREATE INDEX IF NOT EXISTS %s_end_coord ON %s (end_coord);\n CREATE INDEX IF NOT EXISTS %s_strand ON %s (strand);\n", table, table, table, table, table, table, table, table, table, table, table, table, table);
